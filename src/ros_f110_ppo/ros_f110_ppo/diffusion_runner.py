...
def _resolve_workspace_file(relative_path: str) -> Path:
    candidates = [
        Path(__file__).resolve().parents[1] / relative_path,
        Path.home() / "sim_ws" / "src" / "ros_f110_ppo" / relative_path,
    ]
...
MODEL_PATH = _resolve_workspace_file("models/diffusion_policy.pt")
LOG_FILE_PATH = _resolve_workspace_file("logs/diffusion_runtime.csv")
...
class DiffusionRunner(Node):
    ...
    def _build_observation(self):
        ...
        expected_lidar_len = self.observation_dim - 4
        ...
        if len(lidar_normalised) > expected_lidar_len:
            lidar_normalised = lidar_normalised[:expected_lidar_len]
        elif len(lidar_normalised) < expected_lidar_len:
            lidar_normalised = np.pad(
                lidar_normalised,
                (0, expected_lidar_len - len(lidar_normalised)),
                constant_values=1.0,
            )
    ...
    def _compute_safety_speed(self, observation, steering_angle):
        lidar_values = observation[0, :-4].numpy()
        center_index = len(lidar_values) // 2
        forward_lidar = float(
            np.mean(lidar_values[max(0, center_index - 2) : center_index + 2])
        )
